...
def q2rotm(q):
    ...
    q = q.reshape((-1, 4))
    R = torch.zeros([q.shape[0], 9]).to(q.device)
...
def rotm2q(R):
    ...
    L, V = torch.linalg.eig(K) # L: value, V:vector

    index = torch.argmax(torch.real(L), dim=-1)
    length = range(V.shape[0])
    quats = [V[length, 3, index], V[length, 0, index], V[length, 1, index], V[length, 2, index]]
    quats = torch.stack(quats, 1)
    quats = quats.reshape(tuple(raw_shape[:-2] + [4]))
    
    quats = torch.real(quats)
...
def rotm2axangle(rotm):
    ...
    if len(rotm.shape) == 2:
        if (torch.trace(rotm) - 1)/2 >= 1:
            ...
            theta = torch.Tensor([np.pi]).to(device)
        else:
            theta = torch.acos((torch.trace(rotm) - 1)/2)
        if theta == 0:
            theta = torch.Tensor(theta + 0.1).to(device)
        ...
        raw_shape = list(rotm.shape)
        rotm = rotm.reshape([-1, 3, 3])      
        trace = rotm[:, 0, 0] + rotm[:, 1, 1] + rotm[:, 2, 2]
        ac_data = (trace - 1) / 2
        ac_data = torch.clip(ac_data, -1., 1.)
        theta = torch.acos(ac_data).unsqueeze(-1)
    
        # fill = theta.data.new(theta.size()).fill_(0.1)
        fill = torch.ones_like(theta).float() * 0.1
        ...
        # Handle the degenerate cases where theta is divisible by pi
        index0, _ = torch.where(theta % np.pi == 0)   
        ax = ax.clone()     
        for i in range(len(index0)):
            _, _, v = torch.svd(eye3 - rotm[index0[i], ...])
            ax[index0[i], :] = v[:, -1]
...
def q2cont6d(quaternions):
    rotation_mat = q2rotm(quaternions)
    cont_6d = torch.cat([rotation_mat[..., 0], rotation_mat[..., 1]], dim=-1)
    return cont_6d

def cont6d2q(cont_6d):
    matrix = cont6d2matrix(cont_6d)
    axangles = rotm2axangle(matrix)
    quats = axangle2q(axangles)
    return quats



if __name__ == "__main__":
    eul = torch.Tensor([[np.pi/3, np.pi/4, np.pi/6]])
    quats = eul2q(eul, 'zyx')
    print(quats)
    cont6d = q2cont6d(quats)
    print(cont6d)
    quats_re = cont6d2q(cont6d)
    print(quats_re)
