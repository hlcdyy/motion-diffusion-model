...
from data_loaders.humanml.common.bvh_utils import remove_fs, Butterworth
from data_loaders.humanml.common.rotation import wrap, quatbetween, qrot
import numpy as np
...
def main():
    ...
    if args.dataset == 'humanml':
        ...
        # end_second = 103
        # start_second = 127
        # end_second = 130 # waiter
        start_second = 0.1
        end_second = 3  # mocap_0 running happily
        ...
        # args.style_file = 'angry.npy'
        # args.style_file = 'old.npy'
        # args.style_file = 'mummy.npy'
        # args.style_file = 'sore_back.npy'
        # args.style_file = 'waiter.npy'
        # args.style_file = 'happily.npy'
        args.style_file = 'mocap_1.npy'
    ...
    pred_motion_np, _, _, _ = remove_fs("", pred_motion_np, pred_motion_np, BVH_JOINT_NAMES, ee_names, force_on_floor=False, interp_length=2, use_butterworth=True, use_vel3=True, vel3_thr=0.02)
    
    save_file = 'input_style{:02d}.mp4'.format(0)
    animation_save_path = os.path.join(args.save_dir, save_file)
    gt_frames_per_sample = {}
    try:
        plot_3d_motion(animation_save_path, skeleton, pred_motion_np, title='input_style',
    ...
    # for i in range(pred_motion_np.shape[-2]):
    #     for j in range(pred_motion_np.shape[-1]):
    #         pred_motion_np[..., i, j] = Butterworth(pred_motion_np[..., i, j], 1/20, 2.5)
    path = pos2hmlrep(pred_motion_np)
    
    # xz_vel = path[:, 1:3]
    # xz_vel3 = np.zeros((xz_vel.shape[0], 3), np.float32)
    # xz_vel3[:, [0, 2]] = xz_vel
    # plane_vel = np.zeros((3,), np.float32)
    # plane_vel[[0, 2]] = xz_vel.mean(0)
    # qvert = wrap(quatbetween, plane_vel, np.array([0,0,1], np.float32))
    # xz_vel_new = wrap(qrot, qvert, xz_vel3)
    # path[:, 1:3] =  xz_vel_new[:, [0, 2]]
