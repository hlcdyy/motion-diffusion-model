...
class MotionEncoder(nn.Module):
    ...
    def parameters_wo_clip(self):
        return [p for name, p in self.named_parameters() if not name.startswith('mdm_model.')]
    ...
    def forward(self, x, y=None):
        ...
        if y is not None:
            ...
            if y.get('text', None) is not None:
                raw_text = y['text']
                enc_text = self.mdm_model.encode_text(raw_text) 
            else:
                enc_text = None
        else:
        ...
        xseq = self.mdm_model.sequence_pos_encoder(xseq)
    ...
    def finetune_forward(self, nosied_x, timesteps, input_x, y=None):
        ...
        if y is not None and y.get("mask", None) is not None:
            ...
            mask = torch.ones((bs, nframes), dtype=bool, device=nosied_x.device)
...
class MDM(nn.Module):
    ...
    def forward(self, x, timesteps, y=None):
        ...
        y: dictionary if class-conditional. 
        """
        bs, njoints, nfeats, nframes = x.shape
