...
from train.training_loop import FinetuneAELoop
...
def main():
    ...
    if args.dataset in ["bandai-1_posrot", "bandai-2_posrot"]:    
        ...
        data = get_dataset_loader(name=args.dataset, batch_size=args.batch_size, num_frames=args.num_frames, split='train')
        # from model.mdm_forstyledataset import StyleDiffusion
        from model.AE_net import StyleAE
    ...
    for num_file, file in enumerate(test_list):
        ...
        pre_fix = len("Style_" + args.style_file[:-4])
        flag = False
        count = 0
        for ff in exist_files:
            if ff[:pre_fix] == "Style_" + args.style_file[:-4]:
                count += 1
                flag = True
        if count < 78:
            flag = False
        ...
        if args.use_ddim:
            ...
            model, inpainting_diffusion, inpainting_diffusion_ddpm = creat_ddpm_ddim_diffusion(args, ModelClass=StyleAE, timestep_respacing="ddim20")
        else:
            # model, inpainting_diffusion, inpainting_diffusion_ddpm = creat_ddpm_ddim_diffusion(args, ModelClass=StyleDiffusion)
            model, inpainting_diffusion, inpainting_diffusion_ddpm = creat_ddpm_ddim_diffusion(args, ModelClass=StyleAE)
        ...
        print("Training...")
        FinetuneAELoop(args, train_platform, model, data, diffusion=inpainting_diffusion, style_data=data_style).run_loop()
        ...
        for cont_i, cont_file in enumerate(test_list):
            ...
            sample = model(sample_cont, None, None)
