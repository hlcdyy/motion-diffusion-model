...
from train.training_loop import FinetuneDisLoop
...
def main():
    ...
    if args.dataset in ["bandai-1_posrot", "bandai-2_posrot"]:    
        ...
        data = get_dataset_loader(name=args.dataset, batch_size=args.batch_size, num_frames=args.num_frames, split='train')
        # from model.mdm_forstyledataset import StyleDiffusion
        from model.AE_net import StyleDis
    ...
    for num_file, file in enumerate(test_list):
        ...
        if args.use_ddim:
            ...
            model, inpainting_diffusion, inpainting_diffusion_ddpm = creat_ddpm_ddim_diffusion(args, ModelClass=StyleDis, timestep_respacing="ddim20")
        else:
            # model, inpainting_diffusion, inpainting_diffusion_ddpm = creat_ddpm_ddim_diffusion(args, ModelClass=StyleDiffusion)
            model, inpainting_diffusion, inpainting_diffusion_ddpm = creat_ddpm_ddim_diffusion(args, ModelClass=StyleDis)
        ...
        print("Training...")
        FinetuneDisLoop(args, train_platform, model, data, diffusion=inpainting_diffusion, style_data=data_style).run_loop()
