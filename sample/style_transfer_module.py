...
from utils.model_util import creat_style_trans_module, load_model_wo_moenc
...
def main():
    ...
    transfer_model = creat_style_trans_module(args, data)

    print(f"Loading checkpoints from [{args.transfer_module_path}]...")
    
    transfer_model_dict = torch.load(args.transfer_module_path, map_location='cpu')
    load_model_wo_moenc(transfer_model, transfer_model_dict)
        
    # if args.guidance_param != 1:
    #     mdm_model = ClassifierFreeSampleModel(mdm_model)   # wrapping model with the classifier-free sampler

    transfer_model.to(dist_util.dev())
    transfer_model.eval() # disable random masking
    ...
    normal_mu, _ = transfer_model.motion_enc(normal_motion, **normal_kwargs)
    style_mu, _ = transfer_model.motion_enc(style_motion, **style_kwargs)
    tranferred_motion = transfer_model(t2m_motion, normal_mu, style_mu)
    ...
    if transfer_model.motion_enc.mdm_model.data_rep == 'hml_vec':
        ...
        sample_t2m_style = data.dataset.t2m_dataset.inv_transform(tranferred_motion.detach().cpu().permute(0, 2, 3, 1)).float()
    ...
    t2m_source = plot_3d_array([sample_t2m_gt[0][:model_kwargs['y']["lengths"][0]], None, paramUtil.t2m_kinematic_chain, caption[0]])
    imageio.mimsave(os.path.join(out_path, f't2m_source.gif'), np.array(t2m_source), duration=model_kwargs['y']["lengths"][0]/20)
    
    
    t2m_style_trans = plot_3d_array([sample_t2m_style[0][:model_kwargs['y']["lengths"][0]], None, paramUtil.t2m_kinematic_chain, caption[0]+" after_style_trans"])
    imageio.mimsave(os.path.join(out_path, f't2m_style_trans_{0}.gif'), np.array(t2m_style_trans), duration=model_kwargs['y']["lengths"][0]/20)
